...
class cRZCOMDllDirector : public cIGZCOMDirector, public cIGZFrameWorkHooks
{
	...
	protected:
		...
		typedef eastl::vector<cRZCOMDllDirector*> ChildDirectorArray;
		ChildDirectorArray mChildDirectorArray;

		typedef uint32_t ClassObjectID;
		typedef eastl::pair<DummyFunctionPtr, int32_t> FactoryFuncRecord;
		typedef eastl::unordered_map<ClassObjectID, FactoryFuncRecord> ClassObjectMap;
